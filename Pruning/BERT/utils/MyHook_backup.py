...
class SparseColumnPruningRank():
  ...
  def pruning(self, accumulated_scores, sparsity, placeholders, sess, assign_ops, pruning_type, granularity):
    head_num = 12
    size_per_head = 64
    ...
    while layer < len(accumulated_scores):
      if layer % 6 < 3:
        ...
        #mask = np.ones(placeholders[layer].shape)
        #feed[placeholders[layer]]=mask
        #layer += 1
        #continue
...
class NetworkPruningHook(tf.train.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
      ...
      if self.step in self.pruning_time:
        ...
        # ### pruning
        self.ranker.pruning(self.accumulated_scores, sparsity, self.placeholders, run_context.session, self.assign_ops, self.flags.pruning_type, self.flags.granularity)
